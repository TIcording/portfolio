...
var url = 'http://openapi.seoul.go.kr:8088/7776575552616e64313234724a706a72/json/fcltOpenInfo_OLI/1/259/'; /* URL */
// json -> 파일형식
...
xhr.onreadystatechange = function () {
    if (this.readyState == xhr.DONE) {  // <== 정상적으로 준비되었을때
        ...
        // JSON 객체화
        const jsonString2 = parsedJson.fcltOpenInfo_OLI.row;
        ...
        // <img id="residue_favorite" src="./test_img/white_star.png">
        // <button class="depart">출발지</button>
        // <button class="arrive">도착지</button>
        // `
        // const favorite = document.getElementById('residue_favorite')
        ...
        //     }
        // })
        // favorite.addEventListener('click', residue_star)
        // function residue_star() {
        //     if(favorite.src.endsWith('white_star.png')){
        //         favorite.src = './test_img/yellow_star.png'
        //         console.log('즐겨찾기 추가')
        //     }else{
        ...
        window.global_residue_name = facility
        let residue_markers = []
        var imageSrc = "./마커/yello.png";
                            ...
                            if (information.style.display == 'none') {
                                ...
                                for (let i = 0; i < facility.length; i++) {
                                    if (facility[i] == marker.Gb) {
                                        marker_name.innerHTML = `<img src="./test_img/home.png" alt="장애인거주시설">${marker.Gb}`
                                    ...
                                    `
                                        ...
                                        marker_name.addEventListener('click', () => {
                                            ...
                                            detail_name.innerHTML = `<img src="./test_img/home.png" alt="장애인거주시설">${marker.Gb}<img src="./img/x.png" alt="get_out" class="out_img">`
        ...
        window.global_residue = residue_markers
        ...
        residue_button.addEventListener('click', () => {
            ...
            if (markers_ambulance[0].getMap() && markers_library[0].getMap() && markers_charger[0].getMap() && markers_etc[0].getMap() && markers_medical[0].getMap() && residue_markers[0].getMap()) {
                ...
                residue_button.classList.add('press')
            }

            else if (!markers_etc[0].getMap() && !markers_ambulance[0].getMap() && !markers_charger[0].getMap() && !markers_library[0].getMap() && !markers_medical[0].getMap()) {
                for (let i = 0; i < markers_etc.length; i++) {
                ...
                for (let i = 0; i < residue_markers.length; i++) {
                    residue_markers[i].setMap(map)
                }
                for (let i = 0; i < markers_library.length; i++) {
                    markers_library[i].setMap(null)
                }
