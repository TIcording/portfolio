...
var url = 'http://openapi.seoul.go.kr:8088/59447a5655616e643933634d636556/json/fcltOpenInfo_OE/1/22/'; /* URL */
// json -> 파일형식
...
xhr.onreadystatechange = function () {
    if (this.readyState == xhr.DONE) {  // <== 정상적으로 준비되었을때
        ...
        // JSON 객체화
        const jsonString2 = parsedJson.fcltOpenInfo_OE.row;
        ...
        window.global_etc_name = facility


		let etc_markers = []
		var imageSrc = "./마커/red.png";
                          ...
                          if (information.style.display == 'none') {
                            ...
                            for (let i = 0; i < facility.length; i++){
                                if (facility[i] == marker.Gb){
                                    marker_name.innerHTML = `<img src="./test_img/etc.png" alt="기타 시설">${marker.Gb}`
                                    marker_info.innerHTML = `<p>주소: ${address[i]}</p>
                                    <p>전화번호: ${tel[i]}</p>`
                                    get_button.innerHTML = `
                                    <img id="etc_favorite" src="./test_img/white_star.png">
                                    <button class="depart" onclick="addDepart()">출발지</button>
                                    <button class="arrive" onclick="addArrive()">도착지</button>
                                    `

                                    const favorite = document.getElementById('etc_favorite')
                                    favorite.addEventListener('click', etc_star)
                                    // function etc_star() {
                            ...
                            // -------------------------------------------------------------------------------------------
                                    ...
                                    marker_name.addEventListener('click', () => {
                                        ...
                                        detail_name.innerHTML = `<img src="./test_img/etc.png" alt="기타 시설">${marker.Gb}<img src="./img/x.png" alt="get_out" class="out_img">`
        ...
        etc_button.addEventListener('click', () => {
            ...
            if (markers_ambulance[0].getMap() && markers_library[0].getMap() && markers_charger[0].getMap() && etc_markers[0].getMap() && markers_medical[0].getMap() && markers_residue[0].getMap()) {
                ...
                etc_button.classList.add('press')
            }

            else if (!markers_library[0].getMap() && !markers_ambulance[0].getMap() && !markers_charger[0].getMap() && !markers_medical[0].getMap() && !markers_residue[0].getMap()) {
                for (let i = 0; i < markers_library.length; i++) {
                ...
                for (let i = 0; i < etc_markers.length; i++) {
